/**
 * GANDIA 7 — tramitesService.ts (React Native)
...
export async function getMunicipios(): Promise<MunicipioResumen[]> {
  ...
  return munis.map((m: any) => {
    const tramitesDeMuni = (tramites ?? []).filter((t: any) => {
      try { return JSON.parse(t.nombre ?? '').municipioId === String(m.id) } catch { return false }
    })
    const estatusDe = (t: any): TramiteEstatus =>
...
export async function getExpediente(tramiteId: string): Promise<ExpedienteDetalle> {
  ...
  const eventos: EventoUI[] = allEvents.map((e: Evento & { estados?: Estado }) => ({
    id:          String(e.id),
  ...
  const evidencias: EvidenciaUI[] = (evidenciasRaw ?? []).map((ev: Evidencia) => ({
    id:         String(ev.id),
