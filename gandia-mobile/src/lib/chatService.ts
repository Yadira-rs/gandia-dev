/**
 * GANDIA — chatService.ts (React Native)
 * Capa de datos para el chat: conversations + messages sobre Supabase.
 * El Chat nunca toca Supabase directamente — todo pasa por aquí.
 *
 * Uso:
...
// ─────────────────────────────────────────────────────────────────────────────
// HELPER: obtener user_id activo (lee de AsyncStorage vía Supabase)
// ─────────────────────────────────────────────────────────────────────────────
 ...
 *
 * NOTA: En React Native debes crear un objeto File/Blob compatible
 * con Supabase (por ejemplo usando fetch/Blob o libs de Expo).
 */
 ...
 * }
 *
 * NOTA: En React Native la compatibilidad con streams (ReadableStream)
 * depende de la versión de RN/Expo, por lo que aquí mantenemos una
 * implementación básica y un modo "onChunk" opcional.
 */
async function callAcipe(
  ...
  const user_id     = await getActiveUserId()
  const BACKEND_URL = process.env.EXPO_PUBLIC_ACIPE_URL ?? ''

  if (!BACKEND_URL) {
    throw new Error('EXPO_PUBLIC_ACIPE_URL no está definida en el entorno.')
  }
  ...
  // ── Con streaming (si la plataforma lo soporta) ───────────────────────────
  ...
  if (!('body' in res) || !res.body) {
    // Fallback: tratar como respuesta completa si el stream no existe
  ...
  // En entornos que soporten ReadableStream (web / algunas builds Expo)
...
>>>>>>> 00c13da46574878dec7e4b14aedf04cf582696f8
